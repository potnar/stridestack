  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const records = allMessages
        .filter((m: any) => m.type === "record")
        .map((m: any) => m.data);
      const sessions = allMessages
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        if (position_lat !== undefined && position_long !== undefined) {
          ...
          const lat =
            Math.abs(position_lat) > 180
              ? position_lat * (180 / Math.pow(2, 31))
              : position_lat;
          const lng =
            Math.abs(position_long) > 180
              ? position_long * (180 / Math.pow(2, 31))
              : position_long;
          path.push({ lat, lng });
        }
        ...
        if (distanceInSegment >= 100) {
          ...
          if (timeElapsedMs > 0) {
            ...
            const paceMinPerKm =
              ((timeSeconds / distanceInSegment) * 1000) / 60;
      ...
      const activitySummary: ActivitySummary = {
        ...
        avgPace:
          totalDistance > 0 && sessionTotalTime > 0
            ? `${avgPaceMin}:${avgPaceSec.toString().padStart(2, "0")}`
            : "0:00",
        avgSpeed:
          sessionTotalTime > 0
            ? ((totalDistance / sessionTotalTime) * 3.6).toFixed(1)
            : "0.0",
        path,
