...
import { getWeightHistory, getEarliestWeightDate, deleteWeightEntry } from '@/app/actions'
import { useRouter } from 'next/navigation'
import { format, startOfWeek, endOfWeek, subWeeks, addWeeks, subDays, addDays, startOfDay, endOfDay, isAfter, isBefore } from 'date-fns'
...
export function WeightChart({ lastUpdated }: WeightChartProps) {
    ...
    async function fetchData() {
      ...
      if (viewMode === 'DAILY') {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          const entry = history.find(h => startOfDay(new Date(h.date)).getTime() === day.getTime())
          dailyData.push({
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (weekEntries.length > 0) {
            const avg = weekEntries.reduce((acc, curr) => acc + curr.weight, 0) / weekEntries.length
            weeklyData.push({
  ...
  const canGoNext = () => {
    ...
    if (viewMode === 'DAILY') {
      return isBefore(currentDate, today) && format(currentDate, 'yyyy-MM-dd') !== format(today, 'yyyy-MM-dd')
    } else {
  ...
  const canGoPrev = () => {
    if (viewMode === 'DAILY') {
      ...
      const endOfCurrentPeriod = endOfWeek(currentDate, { weekStartsOn: 1 })
      const startOfPeriod = startOfWeek(subWeeks(endOfCurrentPeriod, 3), { weekStartsOn: 1 })
      const earliestStartOfWeek = startOfWeek(earliestDate, { weekStartsOn: 1 })
    ...
    <div className="bg-card p-4 rounded-2xl">
        ...
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={data}>
            ...
            <Tooltip
              ...
              formatter={(value: any) => [`${value} kg`, viewMode === 'DAILY' ? 'Waga' : 'Średnia']}
            />
            <Line
              type="monotone"
              dataKey="weight"
      ...
      {viewMode === 'DAILY' && historyList.length > 0 && (
        <div className="mt-8 space-y-3">
          <h3 className="text-sm font-semibold text-zinc-400 px-1">Ostatnie wpisy</h3>
          <div className="space-y-2">
            {[...historyList].reverse().map((entry) => (
              <div key={entry.id} className="bg-black/40 rounded-xl p-3 flex items-center justify-between border border-white/5">
                <div className="flex flex-col">
                  <span className="text-white font-medium">{entry.weight} kg</span>
                  <span className="text-xs text-zinc-500">{format(new Date(entry.date), 'PPPP', { locale: pl })}</span>
                </div>
                <button
